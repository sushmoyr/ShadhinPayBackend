...
class CryptoConfigSpringContextTest {
        ...
        .run(
            context -> {
              ...
              assertThat(failure)
                  ...
                  .rootCause()
                  .hasMessageContaining("SHADHINPAY_MASTER_KEY");
            });
  }
  ...
  void stagingProfile_withInvalidKey_failsToStart() {
    runner
        .withPropertyValues(
            "spring.profiles.active=staging", "SHADHINPAY_MASTER_KEY=@@@not base64@@@")
        .run(
            context -> {
              assertThat(context).hasFailed();
  ...
  void prodProfile_withValidKey_startsCleanly() {
    ...
    runner
        .withPropertyValues("spring.profiles.active=prod", "SHADHINPAY_MASTER_KEY=" + key)
        .run(
            context -> {
