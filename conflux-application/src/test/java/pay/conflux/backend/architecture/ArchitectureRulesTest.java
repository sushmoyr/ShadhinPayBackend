...
class ArchitectureRulesTest {
  ...
  static final ArchRule controller_interfaces_must_carry_request_mapping =
      classes()
          ...
          .and()
          .resideOutsideOfPackage("pay.conflux.backend.common..")
  ...
  static final ArchRule feature_packages_must_not_depend_on_other_feature_internals =
      classes()
          .that()
          .resideInAPackage("pay.conflux.backend..")
          .and()
          .resideOutsideOfPackage("pay.conflux.backend.common..")
    ...
    return new ArchCondition<JavaClass>(
      ...
      public void check(JavaClass item, ConditionEvents events) {
        ...
        for (var dep : item.getDirectDependenciesFromSelf()) {
          ...
          String prefix = "pay.conflux.backend." + depFeature + ".";
  ...
  private static String featureOf(String packageName) {
    if (packageName == null || !packageName.startsWith("pay.conflux.backend.")) {
      return null;
    }
    String tail = packageName.substring("pay.conflux.backend.".length());
