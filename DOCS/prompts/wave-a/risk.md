...
```
You are starting the `conflux-risk` module on branch `phase-1/risk`. This is the FIRST of THREE sequential sub-prompts (3a → 3b → 3c).
...
- conflux-risk/CLAUDE.md
- DOCS/features/risk/PRD.md (full)
- DOCS/features/risk/TECH_SPEC.md §2 (entities)
- conflux-risk/src/main/java/pay/conflux/backend/risk/usecase/ (locked: `EvaluateTransactionUseCase`, `RiskDecision`, `TransactionContext`)

WORK ONLY IN
- conflux-risk/src/main/java/pay/conflux/backend/risk/{entity,repository,usecase.impl,controller,dto,mapper,spec,constant}/...
- conflux-risk/src/test/...
- conflux-application/src/main/resources/db/migration/V1004__risk_schema.sql
- conflux-application/src/test/java/pay/conflux/backend/risk/...

DO NOT TOUCH
- conflux-risk/src/main/java/pay/conflux/backend/risk/usecase/ (locked)
...
- `mvn -pl conflux-risk -am verify` BUILD SUCCESS.
- JaCoCo ≥ 50%.
...
```
You are continuing the `conflux-risk` module on branch `phase-1/risk`. Prompt 3a is committed.
...
- conflux-risk/CLAUDE.md "SpEL is code execution" gotcha — fail-CLOSED, sanitize EvaluationContext, no T() / no method invocation surface.
...
2. `conflux-risk/pom.xml`: import caffeine.

3. `SafeSpelEvaluator` (`pay.conflux.backend.risk.engine.SafeSpelEvaluator`):
...
4. `CompiledRuleCache` (`pay.conflux.backend.risk.engine.CompiledRuleCache`):
...
5. `BlacklistCache` (`pay.conflux.backend.risk.engine.BlacklistCache`):
...
6. `VelocityCounter` (`pay.conflux.backend.risk.engine.VelocityCounter`):
...
7. `RiskConfig` (`pay.conflux.backend.risk.config.RiskConfig`):
   - Defines the SpEL evaluator's `ExecutorService` bean (small, `Executors.newFixedThreadPool(8)`, daemon threads).
   - Defines `flagThreshold` from `application.yml` (`conflux.risk.flag-threshold`, default 50).
...
```
You are completing the `conflux-risk` module on branch `phase-1/risk`. Prompts 3a + 3b are committed. Your sub-scope: the cross-module `EvaluateTransactionUseCaseImpl` pulling everything together, the case-management endpoints, the latency benchmark, and the final coverage push.

READ FIRST
- conflux-risk/src/main/java/pay/conflux/backend/risk/usecase/EvaluateTransactionUseCase.java + RiskDecision.java + TransactionContext.java (locked from Phase 0)
...
4. Latency benchmark (`conflux-risk/src/test/java/pay/conflux/backend/risk/benchmark/RiskLatencyBenchmark.java`):
...
- JaCoCo ≥ 80% line / 70% branch on `conflux-risk`.
