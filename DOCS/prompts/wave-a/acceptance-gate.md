...
4. **`ApplicationModules.verify()` green**
   - `mvn -pl shadhinpay-application -am test -Dtest=ModularityTests`. Verify the regenerated `target/spring-modulith-docs/` shows the new use-case impls inside their owning modules with no leakage.
