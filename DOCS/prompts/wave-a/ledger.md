...
```
You are starting Phase 1 Wave A on the `shadhinpay-ledger` module. This is the FIRST of THREE sequential sub-prompts (2a → 2b → 2c) on branch `phase-1/ledger`.
...
- shadhinpay-ledger/CLAUDE.md
- DOCS/features/ledger/PRD.md (full)
- DOCS/features/ledger/TECH_SPEC.md (full — invariant-heavy)
- shadhinpay-ledger/src/main/java/com/shadhinpay/ledger/usecase/ (locked: `RecordJournalEntryUseCase`, `GetAccountBalanceUseCase`, `JournalEntryRequest`, `PostingRequest`)
- shadhinpay-common/src/main/java/com/shadhinpay/common/money/Money.java + MoneyConverter.java

WORK ONLY IN
- shadhinpay-ledger/src/main/java/com/shadhinpay/ledger/{entity,repository,usecase.impl,constant}/...
- shadhinpay-ledger/src/test/...
- shadhinpay-application/src/main/resources/db/migration/V1002__ledger_schema.sql
- shadhinpay-application/src/main/resources/db/migration/V1003__ledger_seed_system_accounts.sql
- shadhinpay-application/src/test/java/com/shadhinpay/ledger/...

DO NOT TOUCH
- shadhinpay-ledger/src/main/java/com/shadhinpay/ledger/usecase/ (interfaces/DTOs locked from Phase 0 Prompt 9)
- shadhinpay-payment-core/events/ (locked event records; you only read them in 2b)
...
3. Entities (`com.shadhinpay.ledger.entity`):
...
5. Repositories (`com.shadhinpay.ledger.repository`):
...
7. Per-module routes constants (`com.shadhinpay.ledger.constant.LedgerRoutes`) — empty placeholder for now; controllers come in 2c.
...
- `mvn -pl shadhinpay-ledger -am verify` BUILD SUCCESS.
- JaCoCo ≥ 65%.
...
- shadhinpay-payment-core/src/main/java/com/shadhinpay/paymentcore/events/PaymentCompletedEvent.java (locked record — read constructor)
...
- `shadhinpay-ledger/pom.xml` (TWO dep imports)
...
2. `shadhinpay-ledger/pom.xml`: import both deps.

3. `@EnableRetry` on `shadhinpay-ledger/src/main/java/com/shadhinpay/ledger/config/LedgerConfig.java` (`@Configuration` class, also defines the `ledgerEventExecutor` `TaskExecutor` bean — `corePoolSize=4`, `maxPoolSize=16`, queue capacity 100, thread name prefix `ledger-evt-`).
...
5. Event listener (`com.shadhinpay.ledger.listener.PaymentCompletedEventListener`):
...
```
You are completing the `shadhinpay-ledger` module on branch `phase-1/ledger`. Prompts 2a + 2b are committed. Your sub-scope: REST controllers (merchant + admin), the daily integrity job, the `ListJournalEntriesUseCase` and `VerifyTrialBalanceUseCase` (internal — not cross-module), final coverage push to ≥ 80%.
...
2. REST controllers (`com.shadhinpay.ledger.controller`):
...
5. Specs (`com.shadhinpay.ledger.spec.JournalEntrySpec`).

6. Background job (`com.shadhinpay.ledger.job.LedgerIntegrityJob`):
   ...
   - Calls `VerifyTrialBalanceUseCase.execute()`. On any inequality:
     ...
     - Increment Micrometer counter `shadhinpay.ledger.integrity.failures`.
   - On clean pass: log INFO with the global SUM=0 confirmation.

7. Coverage push: open JaCoCo, identify gaps, add tests until ≥ 80% line / ≥ 70% branch on `shadhinpay-ledger`.
...
- JaCoCo ≥ 80% line, ≥ 70% branch on `shadhinpay-ledger`.
- `mvn -pl shadhinpay-ledger -am verify` BUILD SUCCESS.
