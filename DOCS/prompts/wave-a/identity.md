...
```
You are starting Phase 1 Wave A on the `conflux-identity` module. This is the FIRST of THREE sequential sub-prompts (1a → 1b → 1c) that build the module on branch `phase-1/identity`. After this sub-prompt commits, the next session runs Prompt 1b on the same branch.
...
- conflux-identity/CLAUDE.md
- DOCS/features/identity/PRD.md §1–§4.1 (lifecycle), §4.4 (MFA — for awareness only)
- DOCS/features/identity/TECH_SPEC.md (full)
- conflux-common/src/main/java/pay/conflux/backend/common/crypto/AesGcmCipher.java (you'll wrap this in an AttributeConverter)
- conflux-common/src/main/java/pay/conflux/backend/common/util/IdentifierDetector.java + IdentifierType.java

WORK ONLY IN
- conflux-identity/src/main/java/pay/conflux/backend/identity/{entity,repository,usecase,usecase.impl,controller,dto,mapper,constant}/...
- conflux-identity/src/test/...
- conflux-application/src/main/resources/db/migration/V1001__identity_schema.sql
- conflux-application/src/test/java/pay/conflux/backend/identity/...

DO NOT TOUCH
- conflux-common/ (read-only)
- conflux-identity/src/main/java/pay/conflux/backend/identity/events/ (records locked in Phase 0)
...
2. Entities (`pay.conflux.backend.identity.entity`):
...
3. JPA AttributeConverters (`pay.conflux.backend.identity.entity.converter`):
...
4. Repositories (`pay.conflux.backend.identity.repository`):
...
7. Per-module routes constants (`pay.conflux.backend.identity.constant.IdentityRoutes`):
...
8. Mappers (`pay.conflux.backend.identity.mapper`) — MapStruct, `componentModel = "spring"`. `UserMapper`, `MerchantProfileMapper`. Sensitive fields (`passwordHash`, `mfaSecret`, raw `kycData`) MUST NOT be in any DTO; verify via mapper test.
...
- `mvn -pl conflux-identity -am verify` BUILD SUCCESS.
- JaCoCo line coverage ≥ 70% on `conflux-identity` (full 80% comes by end of 1c).
...
```
You are continuing the `conflux-identity` module on branch `phase-1/identity`. Prompt 1a is merged into this branch as a prior commit. Your sub-scope: KYC submission, admin verify/reject, user blocking, and the two Modulith event publications (`MerchantVerifiedEvent`, `UserBlockedEvent`).

READ FIRST
- The same docs as 1a, plus:
- conflux-identity/src/main/java/pay/conflux/backend/identity/events/MerchantVerifiedEvent.java + UserBlockedEvent.java (locked record shapes — read the constructors carefully)
...
4. Specs (`pay.conflux.backend.identity.spec`): `MerchantSpec` for admin filtering (status, search, date range).
...
```
You are completing the `conflux-identity` module on branch `phase-1/identity`. Prompts 1a and 1b are merged into this branch. Your sub-scope: TOTP-based MFA (enable + verify) and the final coverage push to ≥ 80%.

This sub-prompt is the ONLY one in Wave A authorized to add a root-pom dependency, per cross-cutting decision #3. You will add `dev.samstevens.totp:totp:1.7.1` to root pom `<dependencyManagement>` and import it from `conflux-identity/pom.xml`. No other root-pom edits permitted.
...
- Your current JaCoCo report (`mvn -pl conflux-identity verify` then open `target/site/jacoco/index.html`)

WORK ONLY IN — same as 1a/1b, plus:
- Root `pom.xml` `<dependencyManagement>` (ONE entry: totp 1.7.1)
- `conflux-identity/pom.xml` (ONE dep import: totp)
...
2. `conflux-identity/pom.xml`: add the import (no version — managed at root).
...
7. **Coverage push:** run `mvn -pl conflux-identity verify`, open the JaCoCo report, identify any class < 80% line coverage. Add unit tests until the module reports ≥ 80% line and ≥ 70% branch coverage. Prefer:
...
- JaCoCo ≥ 80% line, ≥ 70% branch on `conflux-identity`.
- `mvn -pl conflux-identity -am verify` BUILD SUCCESS.
