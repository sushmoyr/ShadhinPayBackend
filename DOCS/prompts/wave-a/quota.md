...
```
You are starting the `shadhinpay-quota` module on branch `phase-1/quota`. This is the FIRST of TWO sequential sub-prompts (4a → 4b).
...
- shadhinpay-quota/CLAUDE.md
- DOCS/features/quota/PRD.md (full)
- DOCS/features/quota/TECH_SPEC.md (full)
- shadhinpay-quota/src/main/java/com/shadhinpay/quota/usecase/ (locked: `ReserveQuotaUseCase`, `ConfirmQuotaUseCase`, `ReleaseQuotaUseCase`, `GetUsageUseCase`, `QuotaReservation`, `QuotaUsageView`)

WORK ONLY IN
- shadhinpay-quota/src/main/java/com/shadhinpay/quota/{entity,repository,usecase.impl,cache,config,constant}/...
- shadhinpay-quota/src/test/...
- shadhinpay-application/src/main/resources/db/migration/V1005__quota_schema.sql
- shadhinpay-application/src/test/java/com/shadhinpay/quota/...

DO NOT TOUCH
- shadhinpay-quota/src/main/java/com/shadhinpay/quota/usecase/ (locked)
...
2. Entity (`com.shadhinpay.quota.entity.QuotaUsage extends Auditable`) — `@Version` for write-behind from the monthly job (4b).

3. Repository — `QuotaUsageRepository.findByMerchantIdAndPeriod(UUID, String)`, paginated for admin (4b).

4. Cache port (`com.shadhinpay.quota.cache.QuotaCachePort`) — interface:
...
6. Use-case implementations (`com.shadhinpay.quota.usecase.impl`):
...
7. `QuotaConfig` — `@Configuration` exposing:
   - `freeQuotaPerMonth = 10` from `application.yml` `shadhinpay.quota.free-per-month`.
   - The Lua scripts as `RedisScript<...>` beans.

8. Per-module routes constants (`com.shadhinpay.quota.constant.QuotaRoutes`) — empty placeholder; controllers come in 4b.
...
- `mvn -pl shadhinpay-quota -am verify` BUILD SUCCESS.
- JaCoCo ≥ 65%.
...
```
You are completing the `shadhinpay-quota` module on branch `phase-1/quota`. Prompt 4a is committed. Your sub-scope: REST controllers, the monthly persistence job, the leaked-reservation cleanup job, and the final coverage push.
...
2. Background jobs (`com.shadhinpay.quota.job`):
...
- JaCoCo ≥ 80% line / 70% branch on `shadhinpay-quota`.
- ArchUnit + Modulith green.
- gitleaks, Spotless, PMD clean.
