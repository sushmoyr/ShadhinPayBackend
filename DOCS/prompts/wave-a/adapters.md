...
```
You are starting the `shadhinpay-adapters` Wave A scope on branch `phase-1/adapters`. This is the FIRST of TWO sub-prompts (5a → 5b).
...
- shadhinpay-adapters/CLAUDE.md
- DOCS/features/adapters/PRD.md (full)
- DOCS/features/adapters/TECH_SPEC.md (full)
- shadhinpay-adapters/src/main/java/com/shadhinpay/adapters/port/ (all locked types)
- shadhinpay-adapters/src/main/java/com/shadhinpay/adapters/support/ (HttpClientFactory, NoopTokenService, TokenService, PaymentProviderRegistry)
- shadhinpay-adapters/src/test/.../NoopTokenServiceTest.java (you will delete this)

WORK ONLY IN
- shadhinpay-adapters/src/main/java/com/shadhinpay/adapters/{mock,support}/...
- shadhinpay-adapters/src/test/...
- shadhinpay-application/src/test/java/com/shadhinpay/adapters/...
...
1. `MockAdapter` (`com.shadhinpay.adapters.mock.MockAdapter`):
   ...
   - `initiate(VendorPaymentRequest req, VendorCredentials creds)`:
     - Read `req.metadata().get("mock_outcome")`. Switch:
       ...
       - default (no key) → `VendorResponse(INITIATED, "MOCK-" + req.transactionId(), "https://mock.shadhinpay.local/pay/MOCK-" + req.transactionId(), "{}", null)`.
...
2. `VendorAuthClient` interface + `MockVendorAuthClient` (`com.shadhinpay.adapters.support`):
...
3. `RedisTokenService implements TokenService` (`com.shadhinpay.adapters.support.RedisTokenService`):
...
4. `MockErrorMapper implements ErrorMapper` (`com.shadhinpay.adapters.support.ErrorMapper` is a NEW interface you create):
...
5. **Delete** `shadhinpay-adapters/src/main/java/com/shadhinpay/adapters/support/NoopTokenService.java` AND `shadhinpay-adapters/src/test/.../NoopTokenServiceTest.java`. Reason: replaced by `RedisTokenService`. Document the deletion in the commit message.
...
- `mvn -pl shadhinpay-adapters -am verify` BUILD SUCCESS.
- JaCoCo ≥ 70%.
...
- shadhinpay-adapters/CLAUDE.md "isolated OkHttpClient" gotcha
- Resilience4j docs (Context7 MCP if available): focus on `CircuitBreakerRegistry`, `@CircuitBreaker`, `Decorators.ofCallable(...)`.
- The 5a commits

DELIVERABLES

1. `ResilienceConfig` (`com.shadhinpay.adapters.config.ResilienceConfig`):
...
2. `AdapterResilience` (`com.shadhinpay.adapters.support.AdapterResilience`):
...
3. WireMock test harness (`shadhinpay-application/src/test/java/com/shadhinpay/adapters/wiremock/`):
   ...
   - Self-test: a single integration test in `shadhinpay-application/src/test/.../adapters/MockTokenRefreshIT.java` that:
...
4. **Isolation test** (`shadhinpay-application/src/test/.../adapters/HttpClientIsolationIT.java`):
...
5. **Circuit-breaker test** (`shadhinpay-application/src/test/.../adapters/CircuitBreakerIT.java`):
...
6. Coverage push to ≥ 80% on `shadhinpay-adapters`.
...
- JaCoCo ≥ 80% on `shadhinpay-adapters`.
